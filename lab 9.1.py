...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.speed = 5
        self.weight = random.randint(1, 3)  #Random weight for coins
...
while running:
    ...
    for coin in coins_hit:
        coins_collected += coin.weight  #Add coin weight to score
        new_coin = Coin()
        all_sprites.add(new_coin)
        coin_sprites.add(new_coin)
    ...
    #Increase enemy speed based on coin count
    if coins_collected > 0 and coins_collected % 10 == 0:
        enemy.speed += 0.1
